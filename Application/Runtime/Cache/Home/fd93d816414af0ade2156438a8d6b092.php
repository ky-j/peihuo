    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort peihuo-table">
            ...
            <?php if(is_array($categoryList)): $i = 0; $__LIST__ = $categoryList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                    ...
                    <td class="td-manage"><button class="btn btn-secondary size-MINI radius peihuo-edit" attr-id="<?php echo ($vo["category_id"]); ?>" attr-a="category" attr-message="修改" onclick="layer_show('修改菜品分类','index.php?c=category&a=edit&id=<?php echo ($vo["category_id"]); ?>','',400)">修改</button> | <button class="btn btn-danger size-MINI radius peihuo-delete" attr-id="<?php echo ($vo["category_id"]); ?>" attr-a="category" attr-message="删除">删除</button></td>
