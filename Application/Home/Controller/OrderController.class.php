...
//            exit();
            ...
            // 更改状态为“已配送”之前应确保配送数量都填写了
            if($data['status'] == 2){
                $num = count($_POST['order_number']);
                for($i=0; $i<$num; ++$i) {
                    if($_POST['order_number'][$i]){
                        if($_POST['delivery_number'][$i] == ""){
                            return show_msg(0, '需填写完整的配送数量才能标记为已配送');
                        }
                    }
                }
            }
...
//            return show_msg(1,'更新成功');
    ...
    public function setStatus()
    {
        ...
        // 标记已配送时，检查配送数量是否都填了
        if($data['status'] == 2){
            $res = D("Detail")->getDetailByOrderId($data['id']);
        ...
        // 终结订单时，确保订单状态为已配送
        if($data['status'] == 3){
            $res = D("Order")->find($data['id']);
            if($res['status'] < 2){
                return show_msg(0, '只有状态为已配送的订单才能进行终结操作');
            }
        }

        return parent::setStatus($data,'Order');
    }
