...
class CommonController extends Controller {
    ...
    private function _init() {
        ...
        if(!$isLogin) {
            // 跳转到登录页面
            $this->redirect('/peihuo/index.php?c=login');
        }
    }
    ...
    public function listorder($model='') {
        ...
        try {
            if ($listorder) {
                ...
                if ($errors) {
                    return show_msg(0, '排序失败-' . implode(',', $errors), array('jump_url' => $jumpUrl));
                }
                return show_msg(1, '排序成功', array('jump_url' => $jumpUrl));
            }
        }catch (Exception $e) {
            return show_msg(0, $e->getMessage());
        }
        return show_msg(0,'排序数据失败',array('jump_url' => $jumpUrl));
    }
